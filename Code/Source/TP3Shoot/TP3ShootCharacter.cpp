...
void ATP3ShootCharacter::Fire()
{
    ...
    // Animation, Son, Flash du canon (Simplifié)
    ...
    // --- Calcul du point de départ et de la direction (IA vs JOUEUR) ---
    ...
    if (GetWorld()->LineTraceSingleByChannel(HitResult, Start, End, ECC_Visibility, TraceParams))
    {
        ...
        // Dessine la ligne VERTE (touche l'ImpactPoint)
#if ENABLE_DRAW_DEBUG
        DrawDebugLine(GetWorld(), Start, ImpactPoint, FColor::Green, false, 1.0f, 0, 1.5f);
#endif
    }
    else
    {
    ...
    if (ActualDamage > 0.0f)
    {
        ...
        if (Life <= 0)
        {
            ...
            if (AIController)
            {
                if (UBlackboardComponent* BlackboardComp = AIController->GetBlackboardComponent())
                {
                    BlackboardComp->ClearValue(TEXT("CanSeePlayer"));
                    BlackboardComp->ClearValue(TEXT("PlayerTargetActor"));
                    BlackboardComp->ClearValue(TEXT("LastKnownPlayerLocation"));
                    BlackboardComp->ClearValue(TEXT("IsThreatened"));
                }
